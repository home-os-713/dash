...
- **react-markdown** + **remark-gfm** — render the assistant's streamed Markdown replies cleanly (bold, lists, inline code, tables) instead of dumping raw `**…**`/`-` text. Wrapped in `components/Markdown.tsx`, styled with the warm-editorial semantic tokens. Only used on `/dashboard/assistant`. (Round 16)
...
app/
  ...
  dashboard/analytics/page.tsx   # Portfolio analytics — ONE unified view (no toggle): headline stats + investor ratios + projections + charts + AI insights (Round 16)
  dashboard/assistant/page.tsx   # AI "Ask your portfolio" chat — grounded in real Supabase data; replies render as Markdown; graceful no-key state (Round 16)
...
components/
  ...
  Markdown.tsx               # react-markdown + remark-gfm renderer, themed — used by /dashboard/assistant for model replies (Round 16)
  AddressAutocomplete.tsx    # Places-autocomplete address input (drop-in; falls back to plain input)
  PropertyMap.tsx            # Small pin-only map — component KEPT but no longer rendered on /dashboard/[id] (removed Round 16); still used by add-property flow's coords. Reusable.
...
- **RealPropertyDetail**: Fully redesigned — equity SVG donut, mortgage progress bar, spending breakdown bars, bills list with Add bill modal, Edit property and Edit mortgage modals. All saves are optimistic (local state updated immediately, Supabase updated in background). **Round 16**: the old "Financial summary" expandable block was REPLACED with an in-place `PropertyAnalytics` section that runs the SAME `lib/v0/analytics.ts` engine (`computePropertyMetrics` + `projectProperty`) so per-property cap rate / cash-on-cash / NOI / cash flow / equity match the portfolio analytics page exactly. Point-in-time metrics are labeled **actual**; the compact equity-buildup AreaChart is **projected** under `DEFAULT_ASSUMPTIONS` (link out to /analytics to tune). The `PropertyMap` widget was removed from this page and the hero banner is now full-width (Round 16) — the map component file is kept for reuse.
- **Investor-first dashboard overview (Round 16)**: `/dashboard` headline is now a `PortfolioOverview` card (total value, equity, net monthly cash flow, blended return as stat tiles; property/unit count + occupancy in the header; whole card links to `/dashboard/analytics`). Numbers come from `computePortfolioMetrics` (`lib/v0/analytics.ts`) — in demo mode a `mockAsDbProperties()` helper feeds the engine so the figures still agree with /analytics. Bills/autopay are demoted to a small secondary strip at the bottom of the card. The old one-sentence bill-count hero is gone.
- **Assistant Markdown rendering (Round 16)**: model replies on `/dashboard/assistant` render through `components/Markdown.tsx` (react-markdown + remark-gfm) instead of raw `whitespace-pre-wrap` text. The system prompt in `app/api/assistant/route.ts` was tuned with a FORMATTING block (lead sentence, short paragraphs, flat single-level bullets, bold the key number) to keep replies simple/scannable. Streaming still works (Markdown re-parses the accumulated text on each delta). User bubbles stay plain text.

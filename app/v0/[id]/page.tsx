...
export default function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const bills = billsByProperty[id] ?? [];
  ...
  return (
    <div className="min-h-screen bg-[#2B2B2B] text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        {/* Hero — bills first, single sentence */}
        <div
        ...
        >
          <div className="flex items-start justify-between gap-4 flex-col sm:flex-row sm:items-center">
            <div>
              <p className="text-[11px] uppercase tracking-wider text-white/40 mb-1">{property.location} · {property.type}</p>
          ...
          <CardContent className="space-y-2">
            {sortedBills.map((bill) => {
              ...
              return (
                <div
                  key={bill.id}
                  className={`flex items-center gap-3 p-3 rounded-xl border ${bsc.border} ${bsc.bg} hover:bg-[#4B5436]/10 transition-colors`}
                >
        ...
        {/* Two-column: Spend chart + Financial summary */}
